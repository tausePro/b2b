...
interface ProductoEmpresaOdoo {
   ...
   * direccion. Para otros roles vienen como undefined (el endpoint los
   * elimina antes de responder).
   */
          ...
          <section id="odoo" className="scroll-mt-24 bg-white rounded-xl shadow-sm border border-border overflow-hidden">
            ...
            <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                ...
                <div className="rounded-lg border border-border bg-slate-50 px-3 py-2.5 text-sm text-slate-700">
                  {partnerOdooContext?.pricelist
                    ? `${partnerOdooContext.pricelist.name} (ID ${partnerOdooContext.pricelist.id})`
                    : 'Sin lista asignada en Odoo'}
                </div>
                <p className="mt-2 text-xs text-slate-500">
                  La lista fija se toma directamente del partner en Odoo. Para cambiarla, actualiza el contacto en Odoo.
                </p>
              </div>
                                  ...
                                  <button
                                    ...
                                    onClick={() =>
                                      setVariantsModal({
                                        ...
                                        fallbackPrice: producto.list_price,
                                      })
                                    }
                                    className="mt-1 inline-flex items-center gap-1 rounded-full border border-primary/30 bg-primary/5 px-2 py-0.5 text-[10px] font-semibold text-primary hover:bg-primary/10 transition-colors"
                                    title="Ver variantes con costo, markup y antigüedad por variante"
                                  >
                                    <Layers className="h-3 w-3" />
                                    {producto.product_variant_count} variantes
                                  </button>
                                )}
                              </div>
